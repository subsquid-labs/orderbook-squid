...
var autoStartFuelCore = async (config) => {
  ...
  if (config.autoStartFuelCore) {
    ...
    const { cleanup, snapshotDir } = await (0, import_test_utils.launchNode)({
      args: [
...
async function loadConfig(cwd) {
  ...
  const { forcPath, fuelCorePath } = (0, import_cli_utils.tryFindBinaries)({
...
var configureCli = () => {
  ...
  (command = program.command("init" /* init */)).description("Create a sample `fuel.config.ts` file").addOption(pathOption).option("-w, --workspace <path>", "Relative dir path to Forc workspace").addOption(new import_commander.Option(`-c, --contracts ${arg}`, `${desc} Contracts`).conflicts("workspace")).addOption(new import_commander.Option(`-s, --scripts ${arg}`, `${desc} Scripts`).conflicts("workspace")).addOption(new import_commander.Option(`-p, --predicates ${arg}`, `${desc} Predicates`).conflicts("workspace")).requiredOption("-o, --output <path>", "Relative dir path for Typescript generation output").option("--forc-path <path>", "Path to the `forc` binary").option("--fuel-core-path <path>", "Path to the `fuel-core` binary").option("--auto-start-fuel-core", "Auto-starts a `fuel-core` node during `dev` command").action(withProgram(command, "init" /* init */, init));
  ...
  (0, import_cli.configureCliOptions)(
    program.command("typegen").description(`Generate Typescript from Sway ABI JSON files`)
  );
  (command = program.command("versions")).description("Check for version incompatibilities").addOption(pathOption).action(withBinaryPaths(command, "versions" /* versions */, import_cli2.runVersions));
  return program;
};
