...
var getContractRoot = (bytecode) => {
  ...
  const bytes = (0, import_utils.arrayify)(bytecode);
  const chunks = (0, import_utils.chunkAndPadBytes)(bytes, chunkSize);
  return (0, import_merkle.calcRoot)(chunks.map((c) => (0, import_utils.hexlify)(c)));
};
...
var getContractId = (bytecode, salt, stateRoot) => {
  const root = getContractRoot((0, import_utils.arrayify)(bytecode));
  const contractId = (0, import_hasher.sha256)((0, import_utils.concat)(["0x4655454C", salt, root, stateRoot]));
  return contractId;
};
var hexlifyWithPrefix = (value) => (0, import_utils.hexlify)(value.startsWith("0x") ? value : `0x${value}`);
...
var ContractFactory = class {
  ...
  constructor(bytecode, abi, accountOrProvider = null) {
    this.bytecode = (0, import_utils2.arrayify)(bytecode);
  ...
  async deployContract(deployContractOptions = {}) {
    if (!this.account) {
      throw new import_errors.FuelError(import_errors.ErrorCode.ACCOUNT_REQUIRED, "Cannot deploy Contract without account.");
    }
    ...
    if ((0, import_utils2.isDefined)(setMaxFee) && txCost.maxFee.gt(setMaxFee)) {
      throw new import_errors.FuelError(
  ...
  setConfigurableConstants(configurableConstants) {
    try {
      ...
      Object.entries(configurableConstants).forEach(([key, value]) => {
        ...
        const bytes = (0, import_utils2.arrayify)(this.bytecode);
