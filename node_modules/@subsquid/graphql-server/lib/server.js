...
class Server {
    ...
    module(name) {
        let dir = (0, util_internal_ts_node_1.isTsNode)() ? 'src' : 'lib';
    ...
    async context() {
        ...
        if (await this.customResolvers()) {
            ...
            createOpenreader = () => {
                return new typeorm_1.TypeormOpenreaderContext(dialect, con, con, this.options.subscriptionPollInterval, this.options.log);
            };
        }
        else {
            ...
            createOpenreader = () => {
                return new db_1.PoolOpenreaderContext(dialect, pool, pool, this.options.subscriptionPollInterval, this.options.log);
            };
        }
        return () => {
    ...
    async createPgPool(options) {
        ...
        let { toPgClientConfig } = await Promise.resolve().then(() => __importStar(require('@subsquid/typeorm-config/lib/pg')));
